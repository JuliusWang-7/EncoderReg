...
def dice_val(y_pred, y_true, num_clus):
    if num_clus > 70:
        y_pred = make_one_hot(y_pred.detach().cpu().numpy().squeeze(), num_clus)
        y_true = make_one_hot(y_true.detach().cpu().numpy().squeeze(), num_clus)
        intersection = y_pred * y_true
        intersection = intersection.sum(axis=(1,2,3))
        union = y_pred.sum(axis=(1,2,3)) + y_true.sum(axis=(1,2,3))
        dsc = (2. * intersection) / (union + 1e-5)
        return torch.tensor(dsc.mean())
    else:
